...
export class MessageRelationalRepository implements MessageRepository {
  ...
  async bulkCreate(data: Message[]): Promise<Message[]> {
    const persistenceModel = data.map((d) => MessageMapper.toPersistence(d));
    const newEntities = await this.messageRepository.save(
      this.messageRepository.create(persistenceModel),
    );
    return newEntities.map((entity) => MessageMapper.toDomain(entity));
  }
